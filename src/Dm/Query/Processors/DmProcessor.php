...
namespace Bertshang\Dameng\Query\Processors;
...
class DmProcessor extends Processor
{
    ...
    {
        /** @var \Yajra\Oci8\DmConnection $connection */
    ...
    protected function incrementBySequence(array $values, $sequence)
    {
        ...
        if (! isset($builderArgs[1][0][$sequence])) {
            if ($builder instanceof EloquentBuilder) {
                /** @var \Yajra\Oci8\Eloquent\DmEloquent $model */
    ...
    private function bindValues(&$values, $statement, $parameter)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $statement->bindParam($parameter+1, $values[$i], $type);
            $parameter++;
        }
